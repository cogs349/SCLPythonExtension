...
rule processEachModule
	...
	% TODO for Tom - this doesn't seem to be used?????
...
rule translateTypeStructs Exports [opt export_block]
    ...
    construct SuperInit [repeat statement_or_newline]
	    super().__init__(data) NewLine

    construct body [repeat statement_or_newline]
		SuperInit [addMemberForEachElement SclAdd each Elements]

    construct constructor [repeat statement_or_newline]
		def __init__(self, data): INDENT NewLine
		   body
		DEDENT
	...
	'class UniqueName '( 'PDU '): INDENT NewLine
	    body
	DEDENT
...
function addInteger anElement [struct_element]
	...
	construct Decl [repeat statement_or_newline]
		'self.ShortName = self.ReadFunction() NewLine %IntType ShortName [tolower]
